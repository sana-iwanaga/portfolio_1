            ...
            <ul>
                @forelse($allbooklogs as $log)
                    <li class="mb-4 border-b pb-2">
                        ...
                        <form action="{{ route('booklogs.update', $log->booklog_id) }}" method="POST" class="mt-1">
                            @csrf
                            @method('PUT')
                            ...
                            {{-- メモ --}}
                            <label for="memo-{{ $log->booklog_id }}" class="ml-2">メモ:</label>
                            <input type="text" name="memo" id="memo-{{ $log->booklog_id }}" value="{{ $log->memo }}" class="border rounded p-1 w-64">

                            <button type="submit" class="ml-2 bg-blue-500 text-white px-2 py-1 rounded">更新</button>
                        </form>
                        <a href="{{ route('booklogs.memos', $log->isbn) }}" class="text-blue-600 hover:underline mt-1 inline-block">読書メモを見る</a>
                    </li>
                @empty
                    <li>まだ読書ログはありません</li>
                @endforelse
